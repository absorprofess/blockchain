...
const web3 = new Web3(new Web3.providers
    .HttpProvider('http://192.168.96.130:8545'));
// 1. 拿到 bytecode
...
(async () => {
    ...
    // 3. 创建合约实例并且部署
    console.time('合约部署耗时');
    var result = await new web3.eth.Contract(JSON.parse(interface)).deploy({ data: bytecode, arguments: [['0x41', '0x42', '0x43']] }).send({ from: accounts[0], gas: '1000000' });
